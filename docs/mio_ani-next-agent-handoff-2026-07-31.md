...
- C3 已归档；C4–C9 仍为 `planning`，不得自动启动。
- 播放系统继续延期。
- 用户数据备份、恢复、备份密码继续不实现。
- 不实现登录、注册、云同步。
...
- C3 最终实现提交：`e24da3b feat(persistence): complete C3 Vue migration commit path and Android gates`。
- 相关检查点：`3fec01c`（parser/planner/fingerprint + MioAniDatabase 重命名）、`e8feff4`（有界跨平台缓存基础）。
- C1：`4566700`；C2 已归档；父任务 `07-30-vue-to-flutter-rewrite` 当前为 `3/9 done`。
- C3 已归档到 `.trellis/tasks/archive/2026-08/07-31-persistence-cache-web-migration/`；无活动功能任务，下一项须由用户显式指定（通常为 C4）。
- C3 总成果（单条汇总，不再按窗口罗列）：
  - Schema v2 用户/系统/缓存表、v1→v2 前向迁移、事务回滚与 `clearPublicCache` 边界。
  - 结构化缓存 64 MiB、图片平台预算、过期优先 LRU、90%→75% 回收。
  - 图片：Native 文件 / Web Cache Storage / 内存降级、元数据+304+离线 stale、Web 直显降级与容量策略。
  - Vue 迁移：严格 parser、冲突 planner、稳定指纹、Web-only localStorage reader、单事务幂等 commit、不删旧键、bootstrap 诊断；空 storage 不打开 Drift。
  - 依赖：`path_provider 2.1.6`、`web 1.1.1` 直接依赖；`drift_dev 2.34.0` / `build_runner 2.15.1` 未升级。
  - 验证：`flutter test` 109/109、analyze 零问题、profile APK、web release+Wasm dry run；Android 真机 60 条目录/详情/系统返回、集成测试（portrait surface）、致命日志 0。
  - 用户本轮明确跳过真实 Chrome Vue 幂等/旧键 harness，不得记为已通过。
- C4（07-31-home-schedule）已 `in_progress`，2026-08-04 完成主体实现并提交；C5–C9 仍为 `planning`。
...
执行交接时，工作树可能仍有本地未跟踪文件：

- `.vscode/**` 为本地编辑器配置，默认不提交。
- `web/drift_worker.js.deps` 与 `web/drift_worker.js.map` 是 Worker 编译辅助产物，默认不提交；真正运行所需的 `web/drift_worker.js` 和 `web/sqlite3.wasm` 已提交。
- `git-commit-convention.md` 已纳入版本控制。
- `docs/**` 与 `.trellis/**` 被根 `.gitignore` 忽略；交接与规划默认只本地保存。
...
- C3 规划（已归档）：`.trellis/tasks/archive/2026-08/07-31-persistence-cache-web-migration/{prd,design,implement}.md`。
...
### C3：持久化、缓存与 Web 迁移（已完成并归档）

- 已交付 Schema v2、独立缓存预算/LRU、Native/Web 图片持久缓存、Vue 迁移 commit 路径与 Android 门禁。
- 最终提交 `e24da3b`；真实 Chrome Vue 幂等/旧键 harness 按用户要求本轮跳过。
- 依赖：`path_provider 2.1.6`、`web 1.1.1`；不得升级锁定的 `drift_dev`/`build_runner`。
...
- 领域：`BroadcastSchedule`/`ScheduleDay`/`ScheduleItem`、`ScheduleTime`（`HH:mm` 与 Bangumi 紧凑形式）、`ScheduleWeekday`（周一=1）、周一起始周窗口、稳定排序（已知时间升序→热度→标题→ID）、跨天去重、AniList 标题键匹配（年份±1、集数±6）。
- 数据：`BangumiCalendarSource`（`GET:/calendar`，复用 coordinator 键与目录请求合并）、`AniListScheduleSource`（静态 GraphQL season query + checked DTO）、`ScheduleRepository`（`schedule:week:<周一>:v1`，6h 新鲜/7d 降级，跨本地日期触发后台刷新，AniList 失败保留 Bangumi 基线、Bangumi 失败回退 AniList）、`HomeRepository`（`home:sections:<年-季度>:v1`，45min/7d；catalog/schedule 两分区独立失败，不阻断品牌外壳）。
- 状态/路由：`ScheduleController`（autoDispose family，日期即路由事实）、`HomeController`、强类型 `date` Query（非法值回退今天并规范化），`/schedule?date=` 深链接与详情返回恢复。
- UI：首页 Hero 轮播（焦点进入/页面隐藏/减少动态/手动暂停）、本季推荐/热门/最近更新/放送预览；日程页周导航与七日视图；三档布局与 200% 文本缩放；仅 `MioImage` 加载、Hero 当前+下一有界预取；详情页对 `anilist-` ID 显示占位、不请求 Bangumi。
- 验证：`flutter test` 196/196、`flutter analyze --fatal-infos` 零问题、Profile APK、真机 首页→日程→详情→返回 集成流程、Web Release + 真实 Chrome 深链接/刷新、Windows Release。
- 共享抽象：新增 `AnimeSummaryCodec` 供目录/日程/首页三处缓存复用；`NetworkSource` 增加 `anilistApi`/`anilistImages`，`NetworkUriPolicy.resolveImageSource` 按 host 解析图片源。
- C5 边界：未做发现搜索筛选、分页、人物/评论、追番、导入；日程匹配只做展示增强，不写身份表。
...
- Drift Schema v2、独立结构化/图片缓存预算与淘汰、Native/Web 图片持久化、Vue Web 迁移 commit 路径已落地并归档。
- 最终提交 `e24da3b`；Android 真机与全量单测门禁通过；真实 Chrome Vue 幂等 harness 本轮按用户要求跳过。

### C4：首页与放送日程（2026-08-04 已提交）

- 首页（品牌 Hero/本季推荐/热门/最近更新/放送预览）与独立日程页（`/schedule?date=`）已落地。
- Bangumi 周模板 + AniList `nextAiringEpisode` 展示增强；AniList 匹配不写身份表。
- 缓存键：`schedule:week:<周一>:v1`（6h/7d）、`home:sections:<年-季度>:v1`（45min/7d）；跨日期后台刷新；分区失败独立降级。
- 验证：`flutter test` 196/196、analyze 零问题、Profile APK、真机集成流程、Web Release + Chrome 深链接/刷新、Windows Release。
...
- 实现范围：Schema v2、结构化/图片缓存预算与淘汰、图片管线（Native/Web/元数据/304/离线 stale）、Vue parser/planner/fingerprint/commit/localStorage/bootstrap 诊断。
- 单元/分析：`flutter test` 109/109，`flutter analyze --fatal-infos` 零问题。
- 构建：`flutter build apk --profile`、`flutter build web --release`（含 Wasm dry run）通过。
- Android 真机 M2104K10AC / API 33：USB 安装需用户允许；integration_test 通过（强制 portrait surface）；Profile 加载 60 条 Bangumi 目录、进详情、系统返回恢复；按 PID 过滤 FATAL/ANR/E/flutter/Unhandled/RenderBox 均为 0；Android migration=`notNeeded`。
- 已知工具边界：Flutter 3.44.6/Chrome 150 Test Manager 握手超时；本轮用户跳过真实 Chrome Vue 迁移 harness，不得记为已通过。
- 非阻断：CupertinoIcons 字体提示；MIUI uiautomator `theme_compatibility.xml` 噪声。

### C4 完成汇总（2026-08-04）

- 单元/Widget/集成：`flutter test` 196/196（新增 54 个：日程领域/源/仓库/缓存、首页分区/控制器、路由日期、Hero 预取、页面 widget 三档布局）；`flutter analyze --fatal-infos` 零问题；`dart run build_runner build` 与格式门禁通过。
- 构建：`flutter build apk --profile`（100.3MB）、`flutter build web --release`（Wasm dry run 通过）、`flutter build windows --release` 均成功。
- Android 真机 M2104K10AC / API 33：`integration_test/home_schedule_flow_test.dart` 首页→日程→详情→返回通过，分支与日期状态保留。
- Web：`npx serve -s build/web` + 真实 Chrome（playwright-cli）验证 `/schedule?date=2026-08-11` 深链接与刷新；页面标题 `MioAni`，本次会话 console 无错误（仅有 legacy migration notNeeded 与 Wasm Storage 信息日志）。
- 已知边界：Flutter Web 语义树默认关闭，DOM 快照仅显示 accessibility 占位；日程周切换使用 `context.go`（同分支替换语义，不产生历史条目），详情打开/返回经 Router 栈保留日期。
